...
const GraphView = ({ width, height, data }) => {
    ...
    function drawGraph({ graphSvg, data }) {
        ...
        const svg = d3
            ...
            .attr("transform", function () {
                if (showOptimalOne) {
                    ...
                    return `scale(${scale}, ${scale}) translate(${dagWidth + (svgWidth - dagWidth * scale) / 1.2}, ${
                        dagHeight + margin.y
                    }) rotate(180)`;
                }
            })
        ...
        const links = svg
            ...
            .attr("d", function (d) {
                ...
                newPoints[0][1] = d.source.data.level * (dagHeight / dagDepth) + nodeSize / 2;
                newPoints[d.points.length - 1][0] = d.target.x;
                newPoints[d.points.length - 1][1] = d.target.data.level * (dagHeight / dagDepth) - nodeSize / 2;
    ...
    useEffect(() => {
        if (data.optimizer && data.optimizer.dp)
            setTotalCost(data.optimizer.dp[data.optimizer.dp.length - 1].cheapest_total_paths.total_cost);
