...
const GraphView = ({ width, height, base, dp, cost }) => {
    ...
    const { setShowJoinCard, setNode, setNodeDetails, setJoinOrder, selectedMetric, setSelectedMetric } =
    ...
    function drawGraph({ graphSvg, data }) {
            ...
            .map(d =>
                d.data.nodeData
                    ? d.data.nodeData.total_cost ||
                      (d.data.nodeData.cheapest_total_paths ? d.data.nodeData.cheapest_total_paths.total_cost : 0)
                    : 0
            );
        ...
        const links = svg
            ...
            .attr("d", function (d) {
                ...
                const sourceY = sourceLevel * (dagHeight / dagDepth) + d.source.data.nodeSize / 2;
                const targetY = targetLevel * (dagHeight / dagDepth) - d.target.data.nodeSize / 2;
                ...
                //     newPoints[0][1] = sourceY;
                //     newPoints[newPoints.length - 1][0] = d.target.x;
                //     newPoints[newPoints.length - 1][1] = targetY;
                //     return line(newPoints);
                // } else {
                //     const midY = (sourceY + targetY) / 2;
        ...
        function animate(level) {
            ...
            nodes
                .filter(function (d) {
                    return Math.floor(d.data.level) === level || Math.floor(d.data.level) === level + 1;
                })
                .transition()
                .duration(1000)
                .style("opacity", 1);
            ...
            links
                .filter(function (d) {
                    return Math.floor(d.target.data.level) === level || Math.floor(d.target.data.level) === level + 1;
                })
                .transition()
                .duration(1000)
                .style("opacity", 1)
