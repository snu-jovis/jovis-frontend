...
const JoinOrderTree = ({ width, height, data: relOptInfo }) => {
    ...
    const chosenRelOptInfo = Object.values(relOptInfo).find(info => info.relid === chosen);
    ...
    useEffect(() => {
        ...
        const svg = d3
            ...
            .append("g")
            .attr("transform", `translate(${treeWidth - treeWidth / 3}, ${margin.top})`)
            .call(
        ...
        const gLink = svg.append("g").attr("id", "g-link").attr("fill", "none").attr("stroke", "lightgray");
        ...
        function update(event, source) {
            ...
            nodeEnter
                ...
                .attr("fill", d => (d.children ? d3.schemePastel2[2] : d3.schemePastel1[0]))
            ...
            nodeEnter
                ...
                .attr("dy", function (d) {
                    if (d.depth === root.height && d.parent.children[0].data.relid.length > 8) {
                        if (d.data.relid === d.parent.children[0].data.relid) return "1.2em";
                        else return "0.3em";
                    } else return "0.4em";
                })
        ...
        const groupSums = series[0].map((_, i) => d3.sum(series.map(layer => layer[i][1] - layer[i][0])));
        ...
        rect.selectAll("rect").on("click", function (event, d) {
            ...
            setSelectedCost(`Run Cost: ${d3.format(".0f")(flatTree[i].data.total_cost)} (${percent})`);

            svg.selectAll("circle").transition().duration(500).attr("fill", "ghostwhite");

            svg.select(`#node-circle-${clickedNode}`).transition().duration(500).attr("fill", d3.schemeSet3[3]);
    ...
    return (
        <div>
            ...
            {chosenRelOptInfo ? (
                ...
                >
                    <span className='font-[NanumSquareB] text-base'>Choose a gene to see the join order.</span>
                </div>
            )}
        </div>
    );
